<article class="sermon items-center w-full max-w-xs {{ $hideseries ? 'block border-b border-dashed last:border-b-0' : 'h-32 inline-flex' }}">
    <img src="{{$singlesermon->series->photo ? $singlesermon->series->photo : '/images/series.svg'}}" alt="" class="h-16 w-16 mr-2 object-cover flex-shrink-0 rounded-full {{ $hideseries ? 'hidden' : '' }}">
    <a href="/churches/{{$church->id}}/{{ $pageType }}/sermons/{{$singlesermon->id}}" class="flex-grow p-2 cursor-pointer">
        ...
        <h2 class="font-bold  mb-1 cursor-pointer">{{$singlesermon->title}}</h2>
