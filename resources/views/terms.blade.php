 ...
 <div class="container mx-auto max-w-lg mt-24 leading-normal">
        ...
        <form action="/terms" method="POST" class="w-full bg-white p-4 shadow mb-6" id="terms">
		  ...
		  <label class="flex items-start mb-6">
		    ...
		    <span class="ml-2"> {{ __('We believe that salvation is by grace through faith alone.  That in order to be saved, one must simply turn to God in faith believing what Christ has done for them. (Ephesians 2:8-9)') }}</span>
		  </label>
		  <label class="flex items-start mb-6" >
		    <input type="checkbox" class="form-checkbox mt-1" name="scripture" id="scripture" onchange="checkComplete()">
		    <span class="ml-2"> {{ __('We believe that the Bible is the inspired, perfect and complete word of God. (2 Timothy 3:16-17)') }}</span>
		  </label>
		  <label class="flex items-start mb-6" >
		    <input type="checkbox" class="form-checkbox mt-1" name="punishment" id="punishment" onchange="checkComplete()">
		    <span class="ml-2"> {{ __('We believe that all men are sinners, and that God’s punishment for sin is death and hell. (Romans 3:23)') }}</span>
		  </label>
		  <label class="flex items-start mb-6" >
		    <input type="checkbox" class="form-checkbox mt-1" name="works" id="works" onchange="checkComplete()">
		    <span class="ml-2"> {{ __('We do not believe that baptism, church membership, speaking in tongues, taking of communion, or any other outward work is a necessary sign of saving faith. (Luke 23:43)') }}</span>
		  </label>
		  ...
		  <label class="flex items-start mb-6">
		    ...
		    <span class="ml-2"> {{ __('We will not use ChurchTools Sermons to promote racism, for political electioneering, dispense conspiracy theories or to incite violence.') }}</span>
		  </label>
		  <label class="flex items-start mb-6" >
		    <input type="checkbox" class="form-checkbox mt-1" name="commerce" id="commerce" onchange="checkComplete()">
		    <span class="ml-2"> {{ __('We will not use ChurchTools Sermons to engage in any kind of commerce.') }}</span>
		  </label>
		  <label class="flex items-start" >
		    <input type="checkbox" class="form-checkbox mt-1" name="law" id="law" onchange="checkComplete()">
		    <span class="ml-2"> {{ __('We will not use ChurchTools Sermons in any way that breaks the law, (including copyright laws).') }}</span>
		  </label>	 		
        </form>
